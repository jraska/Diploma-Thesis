...
nad open source build systémem Gradle\footnote{http://gradle.org/ (24.4.2016)}, umožňujícímu definici buildu v Groovy skriptech s možností využití Javy,
...
napovídání, analýzu kódu a celkově vývojářovo pohodlí, neřeší však samotný build, který přenechává Gradlu a spuštění buildu
 ...
 Díky hostování projektu na GitHubu bylo možné snadné využití službu Travis CI
 \footnote{Službu a historii buildů pro aplikaci Asistence cestování si můžeme prohlédnout na \url{https://travis-ci.org/jraska/Diploma-Thesis} (10.4.2016).}
,
...
Jedné se o knihovnu pro objektově relační mapování objektů pro databáze SQLite, které se v
 ...
 třídu a SQL dotazy se píší pomocí generovaných metod a metod knihovny. DBFlow není také ideální
  ...
  adaptér a některé databázové vazby se pomocí anotací nedefinují nejlépe, nicméně pro naše řešení prezentované
...
Nejprve si projdeme obecnou architekruru aplikace a následně popíšeme způsob implementace jednotlivých funkcionalit.
...
Díky specifikům platformy existuje mnoho odlišných přístupů, které se snaží platofrmě Android přizpůsobit
...
dbá aplikace na ot, aby zpráva neobsahovala více než 160 znaků a klient tak zaplatil pouze za jednu SMS.
...
umožní rychlý přístup k aktuální pozici uživatele. Vytvoří se object záměr pro odeslání emailu s předvyplněnou
...
Pro přístup k datům byl použit vzor Repositář~\cite{repository} (Repository), díky kterému
...
V této kapitole si představímeúpravy spojené s jejich zpětnou vazbou a dojmy testerů.
...
\section{Závěr}
V práci jsme nejprve představili platormu Android spolu s dalšími použitými nástroji, které nám pomohly při vývoji.
...
Pokusili jsme se poskytnout lidem s mentálním postižení a jejich asistentům nový nástroj, který by
...
Jako návrhy pro vylepšení mohou být přidání více telefonních čísel na asistenta a implemetaci automatického
...
\bibitem{designpatterns}{Erich Gamma, Richard Helm, Ralph Johnson, John Vlissides}
\textit{Design Patterns: Elements of Reusable Object-Oriented Software}. {Pearson Education, říjen, 1994}
