...
\AddAcronym{MVP}{Model-view-controller}
\AddAcronym{MVC}{Model-view-presenter}
...
V dnešní době jsou již chytré mobilní telefony poměrně snadno dostupnou věcí a spoustu lidí
s mentálním postižením tyto telefony více než zdatně ovládá a nabízí se tedy možnost pokusit se prostřednictvím
 ...
 který je v současné době nejrozšířenějším a také nejdostupnějsím mobilním operačním systémem.
 ...
 za tímto účelem, následně si představíme funkční a grafický návrhy aplikace a samotnou platformu Android.
 ...
 změny a úpravy byly v aplikaci provedeny po obdžení zpětné vazby
...
Po další emailové komunikacijsme se s paní Uhlířovou dohodli, že pro ještě lepší zadání bude
dobré konzultovat nápady s jejími kolegy v Brně a začalo se připravovat setkání, které se následně
v Brně uskutečnilo a zůčastnilo se ho asi deset pečovatelů různých věkových kategorií a také dva klienti
sdružení.

Návrhy jim byly prezentovány a po diskuzi a zpětné vazbě byli ve spolupráci s mým vedoucím definování
aktéři aplikace a vytvořena sada případú užití(use casů), které dávají pro toto zadání smysl a následně
...
GIMP je široce používaný nástroj pro práci s obrázky a jejich úpravu. GIMP znamená
 ...
 množství funkcí apro naše účely byl použit zejména pro úpravu snímků obrazovky užitých v této práci.
 ...
 zobrazení na různých zařízních, pro zdroje platí určitá pravidla a například poměr velikostí variant
 obrázků musí splňovat určitá kritéria. Udržovat a vytvářet tyto zdroje je často velmi obtížné
 a pomoci se nám s tím snaží nástorj Android Asset Studio, což je webová aplikace poskytující
 podporu pro vytáření obrázků v požadovaných velikostech, generování motivů aplikace a podobně.
 Pro naší aplikaci byl nástorj použit zejména pro generování ikon vyskytujících se v aplikaci
...
tradiční architektury jako již zmíněný Model-view-controller, případně podobný Model-view-viewmodel
...
za vrstvou modelu mohou bý  další vrstvy komunikace s databází, případně po síti.
...
Architektura projektu byla tedy volena s přikloněním k MVP, vzhledem k menšímu rozsahu aplikace
...
controllerům k těmto datům pomocí popisovaných objektů \texttt{Observable}. controller, tedy aktivita
...
Vzhledem k nepřesnostecm a šumu při nahrávání však jako cesta často vzniká velmi lomená čára a dojem z vykreslení
...
Aplikace, která chce toto uživateli umožnit, musí do asynchronnních operací přesunou většinu svých déle
...
\subsubsection{Pomalé načtení úvodní obrazovky s cestami}
 ...
 seznamem cest. Bylo tedy využito profilovacího nástroje Traceview, který je součástí Android SDK
  ...
  \begin{figure}[H]
          ...
          \caption{Výstup profilování synchornního načítání cest}
...
Ze stejného profilovacícho býstupu se lze přepnout do záznamů z asynchronního vlákna, který je
...
velmi často odhalit nejen výkonostní problémy, ale také chyby. Žádné zásadní problémy nebyly
...
často složitější a húře čitelný. Během vývoje byl proto využit nástroj
...
tak lze snadno podívat, které metody opravdu zabírají více času a umožňuje nám ujisitit se, že jsou
tyto metody spouštěny v asynchronním vlákně. Ukázku použití můžee vidět na ukázce kódu \ref{hugoshowcase},
...
kterými aplikace prochází, nicméně i přesto se můžou chyby dostat do produkce a je důležité se
 ...
 k odeslání zpětné vazby spolu s informacemi o chybě. testovací uživatelé byli instrováni toto udělat
 a vzhledem k dřívějším testům byla takto nahlášena pouze jedná výjimka.

 Nahlášené chyby si lze poté prohlédnou v konzoli obchodu Google Play, kde se nahrajou také další
...
Jak se během testování ukázalo, někteří uživatelé si při prvním spuštění obrazovky nahrávání nebyli
 ...
 Pro tento účel bylo použito překrytí celé obrazovky poloprůhlednou vrstvou, na které je napsán text instruující
 uživatele, že má klepnout na tlačítko nahrávání a toto tlačítko se nachází v kruhové oblasti nápovědy, která je
...
která byla následně využita k implementaci. Aplikace byla následně publikována a otestována několika uživately pro
...
\end{thebibliography}
  ...
  kde je verze uložená na CD dostupná pod tagem \texttt{1.0.0-thesis-release} v sekci releases.
